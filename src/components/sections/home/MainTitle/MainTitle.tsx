...
const Title = () => {
  ...
  useLayoutEffect(() =>{
    if(screenY == 0){
      ...
      window.addEventListener('wheel', () => {
        setIsVisible(true)
        setTimeout(() => setIsVisible2(true), 1170)
        setTimeout(() =>{
  ...
  return (
    ...
    <AnimatePresence>
      { isVisible && (
      <motion.div initial={{height: 0}} animate={{height: '100vh'}} transition={{duration: 1}} exit={{y: '-100vh'}} className='flex justify-center items-center absolute w-full h-full bg-white overflow-hidden bottom-0'>
      </motion.div>
      )
      }
    </AnimatePresence>
    <AnimatePresence>
      { isVisible2 && (
      <motion.div initial={{height: 0}} animate={{height: '100vh'}} transition={{duration: 1}} exit={{y: '-100vh'}} className='flex justify-center items-center absolute w-full h-full bg-[#191919] overflow-hidden bottom-0 z-10 baz'>
